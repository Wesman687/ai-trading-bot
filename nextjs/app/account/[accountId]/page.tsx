...
export default function AccountPage({ params }: { params: Promise<{ accountId: string }> }) {
  ...
  const { balance, net_pnl, open_trades, config, name, tokens } = account;
      ...
      <Card>
        <CardContent className="grid grid-cols-2 md:grid-cols-4 gap-4 align-center">
          ...
          <div>
            ...
            <p
              className={`text-lg font-bold ${account.available_balance > 0
                  ? "text-green-600"
                  : net_pnl < 0
                    ? "text-red-600"
                    : "text-gray-600"
          ...
          <div className='mb-2'>
            ...
            <p
              className={`text-lg font-bold ${net_pnl > 0
                  ? "text-green-600"
                  : net_pnl < 0
                    ? "text-red-600"
                    : "text-gray-600"
            ...
            {tokens &&
              Object.entries(tokens).map(([token, stats]) => (
                <div key={token} className="flex items-center border-b border-gray-800 pb-1">
                  <p className="text-sm font-semibold text-white w-12">{token}</p>
                  <p className="text-sm text-green-400 w-10 text-right">{stats.win}</p>
                  <p className="text-sm text-red-400 w-10 text-right">{stats.loss}</p>
                  <p className="text-sm text-yellow-300 w-20 text-right">${stats.pnl.toFixed(2)}</p>
                </div>
              ))}
        </CardContent>
      </Card>
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 max-h-[30vh] overflow-y-auto pr-2">
              {open_trades.map((trade: Trade) => (
                ...
                >
                  ...
                  <div
                    className={`text-xs mt-1 ${trade.current_price > trade.entry_price
                        ? "text-green-600"
